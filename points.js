...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Shaper{
        constructor(num){
            this.points = []
            for(let t = 0;t<num;t++){
                let point = new Circle((Math.random()*(canvas.width)), (Math.random()*(canvas.height)), 3, "red")
        ...
        clear(){
            ...
            point.disavg = 0
            for(let t = 0;t<this.points.length;t++){
                point.x+=(this.points[t].x/this.points.length)
                point.y+=(this.points[t].y/this.points.length)
            }
            point.yavg = 0
            point.min = 49000
            point.i = -1
            for(let t = 0;t<this.points.length;t++){
                let link = new LineOP(point, this.points[t])
                let ll = (link.hypotenuse())
                point.disavg += ll/this.points.length
                if(ll < point.min){
                    point.min = ll
                    point.i = t
                }
            ...
            let circ  = new Circle(point.x, point.y, point.disavg, "#00FF0018")
            circ.draw()

        // for(let k = 0;k<this.points.length;k++){
            for(let t = 0;t<this.points.length;t++){
                if(circ.isPointInside(this.points[t])){
                    if(this.points.length > 100){
                        this.points.splice(point.i,1)
                    }
                    break
                }
            }
    ...
    let circle = new Circle(350,350, 300, "transparent")
    let shapp = new Shaper(1000)
    let counter = 0
    function main() {
